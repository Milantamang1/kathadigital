...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Get in touch with Katha Digital. Have a project in mind? Let's talk." },
      { property: "og:title", content: "Contact - Katha Digital" },
...
function ContactPage() {
  ...
  return (
    <>
      <PageHero
        eyebrow="Contact"
        title={<>Have a project in mind? <em className="italic text-gold">Let's talk.</em></>}
          ...
          <div className="space-y-5 lg:col-span-5">
            <div className="mb-7">
              ...
              <p className="mt-4 text-sm leading-6 text-muted-foreground">
                Whether it is a wedding, event, commercial, or original production,
                a clear brief helps us recommend the right crew, timeline, and deliverables.
              </p>
            </div>

            {contactItems.map((item) => (
              <div key={item.label} className="flex items-start gap-4 rounded-2xl border border-border/65 bg-card/55 p-4 shadow-[0_18px_60px_-44px_oklch(0_0_0/0.82)] ring-1 ring-white/5 backdrop-blur-sm transition-all duration-300 hover:border-gold/35 hover:bg-card/70 md:p-5">
                ...
                <div>
                  ...
                  <div className="font-display text-lg leading-snug text-foreground/92">{item.value}</div>
                </div>
              </div>
            ))}
            ...
            <div className="rounded-2xl border border-border/65 bg-card/55 p-4 shadow-[0_18px_60px_-44px_oklch(0_0_0/0.82)] ring-1 ring-white/5 backdrop-blur-sm md:p-5">
              ...
              <div className="flex gap-3">
                {[Instagram, Youtube, Facebook].map((Icon, index) => (
                  <a key={index} href="#" className="flex size-11 items-center justify-center rounded-full border border-border/70 bg-background/30 text-foreground/58 transition-all duration-300 hover:-translate-y-0.5 hover:border-gold/55 hover:bg-gold/10 hover:text-gold">
            ...
            <div className="relative aspect-[4/5] overflow-hidden rounded-[2rem] border border-border/65 bg-[radial-gradient(circle_at_50%_30%,oklch(0.18_0.04_250),oklch(0.06_0.02_60)_70%)] shadow-[0_26px_90px_-52px_oklch(0_0_0/0.9)] ring-1 ring-white/5 sm:aspect-[5/4] lg:aspect-[4/5]">
              <img
                ...
                className="h-full w-full object-contain object-center saturate-[1.06] contrast-[1.03]"
              />
...
function Field({
  ...
  const className = "w-full rounded-2xl border border-border/70 bg-background/50 px-4 py-3 text-sm outline-none shadow-[inset_0_1px_0_oklch(1_0_0/0.04)] transition-colors focus:border-gold/70";
