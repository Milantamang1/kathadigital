...
import { newsPostInputToDb, newsPostToValue } from "@/lib/cms/news";
import { newsPostUpdateSchema } from "@/lib/cms/schemas";
...
export async function PUT(request: Request, context: RouteContext) {
  try {
    ...
    const parsed = newsPostUpdateSchema.parse(await request.json());
    ...
    return apiSuccess({ post: newsPostToValue(post) });
  } catch (error) {
